#!/usr/bin/env python3
"""
GPT-4.1 inference on RAPM 3-few-shot prompts.
Settings match RAPM paper Table 12 (Appendix F.3):
  - temperature: 0.7
  - top_p: 0.9
  - max_tokens: 2048
...
MAX_TOKENS = 2048
CONCURRENCY = 50
INPUT_PRICE = 2.00 / 1_000_000   # GPT-4.1 input
OUTPUT_PRICE = 8.00 / 1_000_000  # GPT-4.1 output
...
async def call_gpt(client, prompt, max_retries=5):
    delay = 2.0
    last_err = None
...
async def run_task(task, concurrency, limit=None):
    in_path = Path(f"{task}_RAP_Top_10_3fewshot.json")
    out_path = Path(f"{task}_gpt41_v2_results.json")
    ckpt_path = Path(f"{task}_gpt41_v2_results.ckpt.json")
    ...
    async def worker(idx):
        async with sem:
            text, usage = await call_gpt(client, prompts[idx]["RAG_prompt"])
